...
public partial class Auth_CntrCurrentBooking : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                dvSpecialService.Visible = false;
                if (Session["_SpecialBookingAllowed"].ToString() == "Y")
                {
                    chkspcl.Checked = false;
                    dvSpecialService.Visible = true;
                }
    ...
    private void datatablestationlist()
    {
        if (gvRouteStationsList.Rows.Count > 0)
        {
            hfSeatStructUp.Value = Session["_SeatLayout"].ToString();
            ScriptManager.RegisterStartupScript(this, this.GetType(), "", "TicketLoad();", true);
    ...
    private void OpenTripDetails(string v)
    {
        ...
        {
            DataTable dt1;
            dt1 = clsCounter.GetOpenTripDetails(Session["p_tripcode"].ToString(), Session["_UserCntrID"].ToString());
    ...
    private void loadRoute()
    {
        try
        {
            ...
            else
            {
                ...
                //txtReceivedAmt.Text = String.Empty;
    ...
    public bool isvalidValues()
    {
        try
        {
            ...
            //DateTime dtFrom = DateTime.ParseExact(txtdepartdate.Text.Trim(), "dd/MM/yyyy", null);
            //if (!DateTime.TryParse(txtdepartdate.Text, out dtFrom))
            //{
           ...
           // -----------------
            ...
            //if (_validation.IsValidInteger(txtReceivedAmt.Text.Trim(), 1, 6) == false)
            //{
            //    Errormsg("Please enter received amount");
            //    return false;
            //}
            //if (_validation.IsValidInteger(txtRefundAmt.Text.Trim(), 0, 6) == false)
            //{
            //    Errormsg("Please enter correct received amount");
            //    return false;
            //}
    ...
    private void GenerateTicket()
    {
        try
        {
            ...
            dt = clsCounter.SaveTicket(p_waybill, p_servicetypeid, p_busregistrationno, p_conductorid, p_departime,
                ...
                p_total_fare, p_fare_per_seat, "C", Session["_UserCntrID"].ToString(), "C", p_from_station_code, intTotSeats,
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dtt1.Rows.Count > 0)
                {
                    btnTripChart.Visible = true;
                    btnTripChart.Enabled = true;
                    openSubDetailsWindow("../E_ticket_current.aspx");
                }
                else
                {
    ...
    private void CloseTrip(string tripcode)
    {
        try
        {
            dt = clsCounter.CloseTrip(tripcode, Session["_CntrCode"].ToString());
            if (dt.TableName == "Success")
            {
                if (dt.Rows.Count > 0)
                {
    ...
    private bool match_layout(string bus_no, string STRP_ID)
    {
        try
        {
            dt = new DataTable(); 
            dt = clsCounter.matchLayout (bus_no ,STRP_ID );
            if (dt.Rows.Count >0)
            {
                if (dt.Rows[0]["layout_same"].ToString().ToUpper()=="YES" )
                {
                    return true;
                }
                else
    ...
    protected void ddljtype_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (match_layout (ddlbus.SelectedValue.ToString(), ddljtype.SelectedValue.ToString() ) ==false )
        {
            Errormsg("The layout assigned to this service and bus are diffrent.<br>Please correct and come again for booking."); 
            return;
        }
        loadRoute();
    }
    ...
    protected void rblYesNo_CheckedChanged(object sender, EventArgs e)
    {
    ...
    protected void btnPrintTicket_Click(object sender, EventArgs e)
    {
        ...
        if (chkspcl.Checked == true)
        {
            ...
            //lblamtreceived.Text = txtReceivedAmt.Text.ToString();
            //lblamtrefund.Text = txtRefundAmt.Text.ToString();
        ...
        else
        {
            ...
            //Session["P_SPLRefNo"] = "";
            Session["Type"] = "T";
            lblConfirmation.Text = "I Have Received Amount " + txtTotalFare.Text + ", Want To Proceed ?";
            mpConfirmation.Show();
        }
    }
    ...
    protected void lbtnspecialyes_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in grvPessenger.Rows)
        {
            ...
            Session["Passenger"] = Session["Passenger"].ToString() + lblseatno.Text.ToString() + "," + txtname.Text.ToString() + "," + ddlidtype.SelectedValue.ToString() + "," + txtidnumber.Text.ToString() + "," + txtidnumber.Text.Substring(txtidnumber.Text.Length - 4) + "," + txtSpclRefno.Text.ToString() + "|" ;

        }
        Session["Passenger"] = Session["Passenger"].ToString().Substring(0, Session["Passenger"].ToString().Length);
        btnPrintTicket.Enabled = false;
        GenerateTicket();
    }
