...
public partial class Auth_Reports_rpt_TC1 : System.Web.UI.Page
{
    ...
    string rpt = System.Web.HttpContext.Current.Server.MapPath("~/Auth/Reports/rpt_TC1.rpt");
    ...
    private void loadBookingCounter(DropDownList ddlcounter)
    {
        try
        {
            ...
            MyCommand.Parameters.Clear();
            DataTable dt;
            MyCommand.Parameters.AddWithValue("@StoredProcedure", "auth.f_get_loadcounter");
            MyCommand.Parameters.AddWithValue("@p_officeid", "0");
            dt = bll.SelectAll(MyCommand);
            if (dt.TableName == "Success")
            {
                if (dt.Rows.Count > 0)
                {
                    ddlcounter.DataSource = dt;
                    ddlcounter.DataTextField = "countername_";
                    ddlcounter.DataValueField = "counterid_";
                    ddlcounter.DataBind();
                }
            }
    ...
    private DataTable LoadData()
    {
        try
        {
            ...
            MyCommand.Parameters.Clear();
            DataTable dt;
            MyCommand.Parameters.AddWithValue("@StoredProcedure", "reports.f_get_rpt_tc1");
            ...
            MyCommand.Parameters.AddWithValue("@p_user", ddlcounter.SelectedValue.ToString());
            dt = bll.SelectAll(MyCommand);
            return dt;
        }
        catch (Exception ex)
    ...
    private void downloadReport()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                cryrpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Context.Response, true, "Counter Daily Current Booking Report-" + DateTime.Now.ToString() + DateTime.Now); ;
            }
            else
