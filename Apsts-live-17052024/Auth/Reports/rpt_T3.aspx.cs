...
public partial class Auth_Reports_rpt_T3 : BasePage
{
    ...
    string rpt = System.Web.HttpContext.Current.Server.MapPath("~/Auth/Reports/rpt_T3.rpt");

    protected void Page_Load(object sender, EventArgs e)
    {
        checkForSecurity();
        Session["_moduleName"] = "Daily Summary";
        if (!IsPostBack)
        {
    ...
    private void loadReport(string rolecode, string _rptype)
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlReport.Items[0].Value = "0";
            ddlReport.SelectedIndex = 0;
            _common.ErrorLog("rpt_T3.aspx-001", ex.Message.ToString());
        }
    ...
    private void loadReportType(string rolecode)
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlReport.Items[0].Value = "0";
            ddlReport.SelectedIndex = 0;
            _common.ErrorLog("rpt_T3.aspx-002", ex.Message.ToString());
        }
    ...
    public DataTable LoadGrid()
    {
        try
        {
            NpgsqlCommand MyCommand = new NpgsqlCommand();
            MyCommand.Parameters.Clear();
            MyCommand.Parameters.AddWithValue("@StoredProcedure", "reports.f_get_rpt_t3");
            MyCommand.Parameters.AddWithValue("p_from_date", txtfrmdate.Text);
            MyCommand.Parameters.AddWithValue("p_to_date", txttodate.Text);
            MyCommand.Parameters.AddWithValue("p_booking_mode", "0");

            DataTable dt = new DataTable();
            dt = bll.SelectAll(MyCommand);
            if (dt.Rows.Count > 0)
            {
                lblsmry.Text = "Total Record <b> " + dt.Rows.Count.ToString() + " </b>" + " | Total Booking Amount <b>" + dt.Rows[0]["tot_booking_amt"].ToString() + " ₹ </b>" + " | Total Cancellation Amount <b>" + dt.Rows[0]["tot_cancellation_amt"].ToString() + " ₹ </b>";
        ...
        {
            _common.ErrorLog("rpt_T3.aspx-003", ex.Message.ToString());
            return null;
        }
    }
    ...
    private void downloadReport()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                cryrpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Context.Response, true, "Daily Summary" + DateTime.Now.ToString() + DateTime.Now); ;
            }
            else
                errorMassage("No Record Found");
        }
        ...
        {
            _common.ErrorLog("rpt_T3.aspx-004", ex.Message.ToString());
        }
    }
    ...
    private void Test()
    {
        ...
        {
            _common.ErrorLog("rpt_T3.aspx-005", ex.Message.ToString());
    ...
    private void ExportGridToExcel()
    {
        ...
        {
            _common.ErrorLog("rpt_T3.aspx-007", ex.Message.ToString());
