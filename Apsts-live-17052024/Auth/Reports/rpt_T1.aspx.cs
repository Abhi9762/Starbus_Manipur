...
public partial class Auth_Reports_rpt_T1 : BasePage
{
    ...
    string rpt = System.Web.HttpContext.Current.Server.MapPath("~/Auth/Reports/rpt_T1.rpt");

    protected void Page_Load(object sender, EventArgs e)
    {
        checkForSecurity();
        Session["_moduleName"] = "Date Wise Online Ticket Booking";
        if (!IsPostBack)
        {
            loadDEPOT();
            loadServicetype();
            txtbookingdate.Text = DateTime.Now.AddDays(0).ToString("dd") + "/" + DateTime.Now.AddDays(0).ToString("MM") + "/" + DateTime.Now.AddDays(0).ToString("yyyy");//DateTime.Now.AddDays(0).ToString("dd/MM/yyyy");
    ...
    private void loadDEPOT()
    {
        ...
        catch (Exception ex)
        {
            ...
            ddldepot.Items[0].Value = "0";
            ddldepot.SelectedIndex = 0;
            _common.ErrorLog("rpt_T1.aspx-001", ex.Message.ToString());
        }
    }
    private void loadServicetype()
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlservicetype.Items[0].Value = "0";
            ddlservicetype.SelectedIndex = 0;
            _common.ErrorLog("rpt_T1.aspx-002", ex.Message.ToString());
        }
    }
    ...
    private void loadReport(string rolecode, string _rptype)
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlReport.Items[0].Value = "0";
            ddlReport.SelectedIndex = 0;
            _common.ErrorLog("rpt_T1.aspx-003", ex.Message.ToString());
        }
    ...
    private void loadReportType(string rolecode)
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlReport.Items[0].Value = "0";
            ddlReport.SelectedIndex = 0;
            _common.ErrorLog("rpt_T1.aspx-004", ex.Message.ToString());
        }
    ...
    public DataTable LoadGrid()
    {
        try
        {
            NpgsqlCommand MyCommand = new NpgsqlCommand();
            MyCommand.Parameters.Clear();
            MyCommand.Parameters.AddWithValue("@StoredProcedure", "reports.f_get_rpt_t1");
            MyCommand.Parameters.AddWithValue("p_bookingdate", txtbookingdate.Text);
            ...
            MyCommand.Parameters.AddWithValue("p_bookingmode", ddlbookingmode.SelectedValue);

            DataTable dt = new DataTable();
            dt = bll.SelectAll(MyCommand);
            if (dt.Rows.Count > 0)
            {
                lblsmry.Text = "Total Record <b> " + dt.Rows.Count.ToString() + " </b>" + " | Total No. of Seat's <b>" + dt.Rows[0]["total_seat"].ToString() + " </b>" + " | Total Fare <b>" + dt.Rows[0]["total_fare"].ToString() + " ₹ </b>" + " | Total Reservation <b>" + dt.Rows[0]["total_reservation"].ToString() + " ₹ </b>" + " | Total Concession <b>" + dt.Rows[0]["total_concession"].ToString() + " ₹ </b>" + " | Total Tax <b>" + dt.Rows[0]["total_tax"].ToString() + " ₹ </b>" + " | Total Amount <b>" + dt.Rows[0]["total_amount"].ToString() + " ₹ </b>";
        ...
        {
            _common.ErrorLog("rpt_T1.aspx-005", ex.Message.ToString());
            return null;
        }
    }
    ...
    private void downloadReport()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                CrystalDecisions.CrystalReports.Engine.TextObject objtxtRPTCancellationDate = (TextObject)cryrpt.ReportDefinition.Sections[0].ReportObjects["txtbookingdate"];
                objtxtRPTCancellationDate.Text = txtbookingdate.Text;
                ...
                CrystalDecisions.CrystalReports.Engine.TextObject objtxtRPTCancellationMode = (TextObject)cryrpt.ReportDefinition.Sections[0].ReportObjects["txtbookingmode"];
                objtxtRPTCancellationMode.Text = ddlbookingmode.SelectedItem.Text;
                ...
                cryrpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Context.Response, true, "Date Wise Online Ticket Booking -" + DateTime.Now.ToString() + DateTime.Now); ;
            }
            else
                errorMassage("No Record is available");
        }
        catch (Exception ex)
        {
            _common.ErrorLog("rpt_T1.aspx-006", ex.Message.ToString());
        }
    }
    ...
    private void Test()
    {
        ...
        {
            _common.ErrorLog("rpt_T1.aspx-007", ex.Message.ToString());
    ...
    private void ExportGridToExcel()
    {
        ...
        {
            _common.ErrorLog("rpt_T1.aspx-008", ex.Message.ToString());
