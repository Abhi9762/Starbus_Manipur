...
public partial class Auth_Reports_rpt_P2 : BasePage
{
    ...
    string rpt = System.Web.HttpContext.Current.Server.MapPath("~/Auth/Reports/rpt_P2.rpt");

    protected void Page_Load(object sender, EventArgs e)
    {
        checkForSecurity();
        Session["_moduleName"] = "Payment Gateway Summary";
        if (!IsPostBack)
        {
            loadpaymentgateway();
    ...
    private void loadpaymentgateway()
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlpayment.Items[0].Value = "0";
            ddlpayment.SelectedIndex = 0;
            _common.ErrorLog("rpt_P2.aspx-001", ex.Message.ToString());
        }
    }
    ...
    private void loadReport(string rolecode, string _rptype)
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlReport.Items[0].Value = "0";
            ddlReport.SelectedIndex = 0;
            _common.ErrorLog("rpt_P2.aspx-002", ex.Message.ToString());
        }
    ...
    private void loadReportType(string rolecode)
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlReport.Items[0].Value = "0";
            ddlReport.SelectedIndex = 0;
            _common.ErrorLog("rpt_P2.aspx-003", ex.Message.ToString());
        }
    ...
    public DataTable LoadGrid()
    {
        try
        {
            NpgsqlCommand MyCommand = new NpgsqlCommand();
            MyCommand.Parameters.Clear();
            MyCommand.Parameters.AddWithValue("@StoredProcedure", "reports.f_get_rpt_p2");
            ...
            if (dt.Rows.Count > 0)
            {
                lblsmry.Text = "Total Record <b> " + dt.Rows.Count.ToString() + " </b>" + " | Transaction <b> " + dt.Rows[0]["total_txn"].ToString() + " </b>" + " | Credited Amount <b> " + dt.Rows[0]["credit_amt"].ToString() + " ₹ </b>" + " | Debited Amount <b> " + dt.Rows[0]["debit_amt"].ToString() + " ₹ </b>" + " | Total Amount <b> " + dt.Rows[0]["totalamt"].ToString() + " ₹ </b>";


                gvPaymentGatewaySummary.DataSource = dt;
                gvPaymentGatewaySummary.DataBind();
        ...
        {
            _common.ErrorLog("rpt_P2.aspx-004", ex.Message.ToString());
            return null;
        }
    }
    ...
    private void downloadReport()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                cryrpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Context.Response, true, "Payment Gateway Summary-" + DateTime.Now.ToString() + DateTime.Now); ;
            }
            else
                errorMassage("No Record Found");
        }
        ...
        {
            _common.ErrorLog("rpt_P2.aspx-005", ex.Message.ToString());
        }
    }
    ...
    private void Test()
    {
        try
        {
            if (gvPaymentGatewaySummary.Rows.Count > 0)
            {
                gvPaymentGatewaySummary.UseAccessibleHeader = true;
                gvPaymentGatewaySummary.HeaderRow.TableSection = TableRowSection.TableHeader;
            }
        }
        catch (Exception ex)
        {
            _common.ErrorLog("rpt_P2.aspx-006", ex.Message.ToString());
    ...
    private bool IsValidValue()
    {
        ...
        {
            _common.ErrorLog("rpt_P2.aspx-007", ex.Message.ToString());
            return false;
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                gvPaymentGatewaySummary.AllowPaging = false;
                ...
                gvPaymentGatewaySummary.HeaderRow.BackColor = Color.White;
                foreach (TableCell cell in gvPaymentGatewaySummary.HeaderRow.Cells)
                {
                    cell.BackColor = gvPaymentGatewaySummary.HeaderStyle.BackColor;
                }

                foreach (GridViewRow row in gvPaymentGatewaySummary.Rows)
                {
                    ...
                    foreach (TableCell cell in row.Cells)
                    {
                        if (row.RowIndex % 2 == 0)
                        {
                            cell.BackColor = gvPaymentGatewaySummary.AlternatingRowStyle.BackColor;
                        }
                        else
                        {
                            cell.BackColor = gvPaymentGatewaySummary.RowStyle.BackColor;
                        }
                        cell.CssClass = "textmode";
                    }
                }

                gvPaymentGatewaySummary.RenderControl(hw);
    ...
    protected void gvPaymentGatewaySummary_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        gvPaymentGatewaySummary.PageIndex = e.NewPageIndex;
        this.LoadGrid();
    }
    ...
    protected void lbtnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (!IsValidValue())
        {
            gvPaymentGatewaySummary.DataSource = null;
            gvPaymentGatewaySummary.DataBind();
            pnlReport.Visible = false;
            pnlMsg.Visible = true;
            return;
