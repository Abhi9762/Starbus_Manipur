...
public partial class Auth_Reports_rpt_P7 : BasePage
{
    ...
    string rpt = System.Web.HttpContext.Current.Server.MapPath("~/Auth/Reports/rpt_P7.rpt");

    protected void Page_Load(object sender, EventArgs e)
    {
        checkForSecurity();
        Session["_moduleName"] = "Refunded Transaction";
        if (!IsPostBack)
        {
            loadpaymentgateway();
    ...
    private void loadpaymentgateway()
    {
        ...
        catch (Exception ex)
        {
            ...
            ddlpayment.Items[0].Value = "0";
            ddlpayment.SelectedIndex = 0;
            _common.ErrorLog("rpt_P7.aspx-001", ex.Message.ToString());
        }
    }
    ...
    public DataTable LoadGrid()
    {
        try
        {
            NpgsqlCommand MyCommand = new NpgsqlCommand();
            MyCommand.Parameters.Clear();
            MyCommand.Parameters.AddWithValue("@StoredProcedure", "reports.f_get_rpt_p7");
            ...
            if (dt.Rows.Count > 0)
            {
                lblsmry.Text = "Total Record <b> " + dt.Rows.Count.ToString() + " </b>" + " | Total Refunded Amount <b> " + dt.Rows[0]["total_refund_amount"].ToString() + " ₹ </b>";
        ...
        {
            _common.ErrorLog("rpt_P7.aspx-004", ex.Message.ToString());
            return null;
        }
    }
    private void downloadReport()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                cryrpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Context.Response, true, "Refunded Transaction-" + DateTime.Now.ToString() + DateTime.Now); ;
            }
            else
                errorMassage("No Record Found");
        }
        ...
        {
            _common.ErrorLog("rpt_P7.aspx-005", ex.Message.ToString());
        }
    }
    private void Test()
    {
        ...
        {
            _common.ErrorLog("rpt_P7.aspx-006", ex.Message.ToString());
    ...
    private bool IsValidValue()
    {
        ...
        {
            _common.ErrorLog("rpt_P7.aspx-007", ex.Message.ToString());
            return false;
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Refunded Transaction-" + DateTime.Now.ToString() + ".xls");
            Response.Charset = "";
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                gvRefund.HeaderRow.BackColor = Color.White;
                foreach (TableCell cell in gvRefund.HeaderRow.Cells)
                {
                    cell.BackColor = gvRefund.HeaderStyle.BackColor;
                }

                foreach (GridViewRow row in gvRefund.Rows)
                {
                    ...
                    foreach (TableCell cell in row.Cells)
                    {
                        if (row.RowIndex % 2 == 0)
                        {
                            cell.BackColor = gvRefund.AlternatingRowStyle.BackColor;
                        }
                        else
                        {
                            cell.BackColor = gvRefund.RowStyle.BackColor;
                        }
                        cell.CssClass = "textmode";
                    }
                }

                gvRefund.RenderControl(hw);
        ...
        {
            _common.ErrorLog("rpt_P7.aspx-010", ex.Message.ToString());
