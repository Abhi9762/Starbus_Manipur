...
/// <summary>
/// Summary description for wsAgentEtm
/// </summary>
[WebService(Namespace = "http://tempuri.org/")]
[WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
// To allow this Web Service to be called from script, using ASP.NET AJAX, uncomment the following line. 
// [System.Web.Script.Services.ScriptService]
public class wsAgentEtm : System.Web.Services.WebService
{
    ...
    classWalletAgent clsag_wallet = new classWalletAgent();
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void agentLogin(string userId, string userPwd, string logindatetime, string imei, string latt, string longg, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //    Context.Response.Clear();
                //    Context.Response.ContentType = "application/json";
                //    string jsonstrr = finalJSON("103", "Result", DataTableToJSON(dtnothing), "This device is not allocated to you. Please contact helpdesk");
                //    Context.Response.Write(jsonstrr);
                //    return;
                //}
                ...
                if (checkPassword(dt.Rows[0]["upwd"].ToString(), userPwd) == true)
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                        row["LOGID"] = logid;
                    if (Convert.ToInt16(dt.Rows[0]["urole"].ToString().Trim()) == 19)
                    {
                        code = "100";
                        msg = "";
                        Context.Response.Clear();
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void RemoveLogin(string userId, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void agentdetails(string userId, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "Agent_Details", DataTableToJSON(dt), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void GenerateRenewRequest(string userId, string Imei, string UPDATEDBY, string EmailId, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            dt = new DataTable();
            dt = obj.Generate_RenewRequest(userId, Imei, UPDATEDBY, EmailId);
            string code = "";
            string msg = "";
            if (dt.Rows.Count > 0)
            {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void dashboard(string userId, string cntrId, string action, string datee, string StonId, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            dtdetail = obj.loadAgentBookingDetails(userId, StonId);

            DataTable dtagwallet = new DataTable();
            dtagwallet = clsag_wallet.GetWalletDetailDt(userId);

            DataTable dtOpenTrips = new DataTable();
            dtOpenTrips = obj.load_OpenTrips(userId);
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "ServiceType", DataTableToJSON(dtServiceType), "Details", DataTableToJSON(dtdetail),
                "Agent_Wallet", DataTableToJSON(dtagwallet), "OpenTrips", DataTableToJSON(dtOpenTrips),
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void getPaymentGateways(string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
    ...
    [WebMethod()]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void Check_wallet_balance(string usercode, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            dt_wallet = clsag_wallet.GetWalletDetailDt(usercode);
            string code = "", msg = "";
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "Wallet_Balance", DataTableToJSON(dt_wallet), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod()]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void getWallet_trans(string userId, string Fromdate, string Todate, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            DataTable dt_wallet = new DataTable();
            dt_wallet = clsag_wallet.getWalletTransactions_dt_bydate(userId, Fromdate, Todate);
            string code = "", msg = "";
            if (dt_wallet.Rows.Count > 0)
            {
                code = "100"; msg = "";
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "WalletTrans", DataTableToJSON(dt_wallet), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod()]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void getWallet_topup_trans(string userId, string Fromdate, string Todate, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            DataTable dt_wallet = new DataTable();
            dt_wallet = clsag_wallet.gettopupTransactions_dt_bydate(userId, Fromdate, Todate);
            string code = "", msg = "";
            if (dt_wallet.Rows.Count > 0)
            {
                code = "100"; msg = "";
    ...
    [WebMethod()]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void walletTopupTxnStatus(string userId, string refNo, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            DataTable dt_wallet = new DataTable();
            dt_wallet = clsag_wallet.gettopupTransactions_status(userId, refNo);
            string code = "", msg = "";
            if (dt_wallet.Rows.Count > 0)
            {
                code = "100"; msg = "";
    ...
    public void wallet_Topup_start(string userId, string amount, string token)
    {
        try
        {
            ...
            DataTable dt_wallet = new DataTable();
            dt_wallet = clsag_wallet.walletTopup_start_completed("0", userId, amount, "F");
            string code = "", msg = "";
            if (dt_wallet.Rows.Count > 0)
            {
                code = "100"; msg = "";
    ...
    public void savePassengers(string depotServiceCode, string tripType, string strpid, string journeyDate, string fromStationId, string toStationId, string booked_by_type_code, string userId, string userMobile, string userEmail, string bordeingStationId, string passengers, string ip_imei, string token)
    {
        try
        {
            ...
            _common.ErrorLog("wsCounterETM", dttt.Rows.Count.ToString());
    ...
    public void updateTicket(string ticketno, string counterid, string usercode, string pmt_mode, string ip_imei, string token)
    {
        try
        {
            ...
            DataTable dt_ = new DataTable("dt_");
            bool xx = obj.agent_updateTkt(ticketno, usercode);
            string code = "", msg = "";
            if (xx == true)
            {

                dt_.Columns.Add("ticketno", typeof(string));
                dt_.Rows.Add(ticketno);
                code = "100"; msg = "";
            }
            else
            { code = "101"; msg = ""; }
    ...
    public void CancelTicket(string TicketNo, string seatNo, string NoOfSeat, string CANCELLEDBY, string cancel_by_type, string token)
    {
        try
        {
            ...
            DataTable dt_ = new DataTable();
            dt_ = trvlclass.cancelTicket(TicketNo, seatNo, "0", Convert.ToInt32(NoOfSeat), CANCELLEDBY, "", cancel_by_type, "W");
    ...
    public void SpclCancelAvailableTicket(string ticketno, string token)
    {
        try
        {
            ...
            DataTable dt_ = new DataTable();
            dt_ = trvlclass.getSplCancelledTicketsNew("0", ticketno, "A");
        ...
        string driver, string driver2, string departime, string routeid, string tripid, string from_station_code, string to_station_code, string trip_date, string received_amt, string refund_amt, string total_fare, string fare_per_seat, string booked_by_type_code, string booked_by_user_code, string booking_mode, string boarding_station_code, string total_seats_booked, string Passenger, string updatedby, string ip_imei, string ConcessionId, string OnlineVerificationYN, string CustpassNumber, string IdVerificationYN, string IdVerification, string DocumentVerificationYN, string DocumentVerification, string SpecialYn, string Book_sub_mode, string Payment_mode, string with_waybill, string token)
    {
        try
        {
            ...
            DataTable dt = new DataTable();
            dt = clscounter.SaveTicketWaybill(waybillno, Convert.ToInt64(servicetypeid), busregistrationno, conductorid, conductorid2, driver, driver2,
                departime, Convert.ToInt64(routeid)
    ...
    public void ConfirmCurrntTicket(string ticket_no, string received_amt, string refund_amt, string token)
    {
        try
        {
            ...
            DataTable dt = new DataTable();
            dt = clscounter.ConfirmTicket(ticket_no, Convert.ToDecimal(received_amt), Convert.ToDecimal(refund_amt));
            string code = "", msg = "";
            if (dt.Rows.Count > 0)
            {
                code = "100"; msg = "";
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "ConfirmTicket", DataTableToJSON(dt), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void CurrentConfirmDetails(string ticketNo, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            DataTable dt = new DataTable();
            dt = obj.getTicketDetails_byCrntTicket(ticketNo);
            string code = "", msg = "";
            if (dt.Rows.Count > 0)
            {
                code = "100"; msg = "";
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "ticketDeatil", DataTableToJSON(dt), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void getAccount_Smry(string userId, string fromdate, string todate, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            dt1 = obj.getAccount_Smry_dt(userId, fromdate, todate);
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "Account_Smry", DataTableToJSON(dt1), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void getTrans_Smry(string userId, string fdate, string tdate, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            DataTable dt1 = new DataTable();
            dt1 = obj.getagTrans_Smry_dt(userId, fdate, tdate);
            ...
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            string jsonstr = finalJSON(code, "Trans_Smry", DataTableToJSON(dt1), msg);
            Context.Response.Write(jsonstr);
        }
        catch (Exception ex)
        {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
    public void agentChange_Pwd(string userId, string oldPwd, string newPwd, string imei, string token)
    {
        try
        {
            SoapHeader.UserAuthenticationToken = token;
            ...
            if (dt.Rows.Count > 0)
            {
                if (checkPassword(dt.Rows[0]["upwd"].ToString(), oldPwd) == true)
                {
                    DataTable dtpass = new DataTable();
                    dtpass = obj.ChngePassword(userId, newPwd);
                    code = "100";
                    msg = "";
                    Context.Response.Clear();
                    Context.Response.ContentType = "application/json";
                    string jsonstr = finalJSON(code, "ChangePwd", DataTableToJSON(dtpass), msg);
                    Context.Response.Write(jsonstr);
                    return;
                }
                else
